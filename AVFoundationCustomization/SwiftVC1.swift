...
//  Kamera
//
//  Created by Mohsin on 26/09/2015.
//  Copyright (c) 2015 Bob McCune. All rights reserved.
//
    ...
    @IBAction func flashMode(sender: UIButton) {
        ...
        if self.cameraController.cameraHasFlash {
            ...
            if self.flashModeCount%3 == 0{
                self.cameraController.flashMode = AVCaptureFlashMode.Auto
            }
            else if self.flashModeCount%3 == 1{
                self.cameraController.flashMode = AVCaptureFlashMode.On
            }
            else if self.flashModeCount%3 == 2{
                self.cameraController.flashMode = AVCaptureFlashMode.Off
            }
        }
         if self.cameraController.cameraHasTorch {
            println("camera has torch")
            if self.flashModeCount%3 == 0{
                self.cameraController.torchMode = AVCaptureTorchMode.Auto
            }
            else if self.flashModeCount%3 == 1{
                self.cameraController.torchMode = AVCaptureTorchMode.On
            }
            else if self.flashModeCount%3 == 2{
                self.cameraController.torchMode = AVCaptureTorchMode.Off
            }
        }
